...
// For drawing the vanishing line
Point p1, p2;
Point points[100];
int pnum = 0;
...
Mat computeVpScore(Mat image_origin, Mat image)
{
	...
	if (p1.x == -1) {
		p1.x = p_max.x / scale_factor_width;
		p1.y = p_max.y / scale_factor_height;
	}
	else {
		p1 = p2;
	}
	p2.x = p_max.x / scale_factor_width;
	p2.y = p_max.y / scale_factor_height;
	points[pnum].x = p2.x;
	points[pnum].y = p2.y;
	pnum++;
...
int drawPointsAndLine(Mat img) {

	for (int i = 0; i<pnum; i++) {
		cv::circle(img, points[i], 10, Scalar(255, 255, 255), 2, 2, 0);
	}
	for (int i = 1; i<pnum; i++) {
		cv::line(img, cvPoint(points[i-1].x, points[i-1].y), cvPoint(points[i].x, points[i].y), Scalar(0), 3, 8, 0);
	}
	return 1;
}
...
int* findDominantEdges(float candidate_x, float candidate_y, bool multiple) {
	...
	for (int i = 2; i < bottom_right; i += 5) {
		int OCR = 0;
...
int segmentRoad(Mat image_origin, Mat image, bool isVideo) {
	...
	if (isVideo) {
		drawPointsAndLine(image_origin);
	}
...
int process(string filePath, bool isVideo) {
	if (isVideo == false) {
		...
		segmentRoad(image_origin, image, isVideo);
		imwrite(".\\results\\" + filePath, image_origin);
		waitKey(0);
	}
	else {
